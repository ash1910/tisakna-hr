...
class PaymentTag extends AbstractTag
{
    public function parse()
    {
        ...
        if ($this->param('order_id') !== false || $this->param('order_hash') !== false) {
            ...
            $order = ee()->store->orders->get_cart();
            if ($order->isEmpty()) {
        ...
        $payment_method = ee()->store->payments->find_payment_method($this->param('payment_method'));
        if (!$payment_method) {
        ...
        if (!empty($payment_method->createGateway()->transparentRedirect)) {
            ...
            $request = ee()->store->payments->process_payment($order, $transaction, null, false);
            try {
            ...
            if ($this->param('return') !== false) {
                $url = ee()->functions->create_url($this->param('return'));
