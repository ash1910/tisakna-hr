...
class DownloadTag extends AbstractTag
{
    public function parse()
    {
        ...
        $order = Order::where('site_id', config_item('site_id'))->where('order_id', $order_id)->first();
        if (empty($order) OR $order->is_order_unpaid) return;

        // make sure download hasn't expired
        if (ee()->store->orders->is_download_expired($order, $expire)) return;

        $file = ee()->store->store->get_file_by_url($file_url);
        if (!$file) {
        ...
        $params['k'] = ee()->store->orders->generate_download_key($order, $file->file_id, $expire);

        $out = '<a href="'.ee()->store->store->get_action_url('act_download_file').AMP.
