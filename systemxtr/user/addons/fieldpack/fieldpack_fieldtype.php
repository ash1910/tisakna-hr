 ...
 */
abstract class Fieldpack_Fieldtype extends \fieldpack\Eeharbor_ft {
	...
	function parse_iterators(&$tagdata)
	{
		...
		// {count} tags
		$tagdata = ee()->TMPL->swap_var_single($this->_count_tag, $this->_iterator_count, $tagdata);
	}
	...
	protected function _include_ce_icon($icon)
	{
		...
		{
			ee()->cp->add_to_head('<style type="text/css">.content_elements_icon_fieldpack_'.$icon.' { background: url('.$this->_theme_url().'images/icons/'.$icon.'.png); background-size: 16px; }</style>');
		}
	}
...
class Fieldpack_Multi_Fieldtype extends Fieldpack_Fieldtype {
	...
	function display_settings($data)
	{
		...
		$this->_display_settings_add_row(
        ...
        return $this->_package_display_settings('field_options_'.$this->class, $this->class);
	}
	...
	{
		$post = ee()->input->post($this->class.'_options');
	...
	function replace_tag($data, $params = array(), $tagdata = FALSE)
	{
		...
		if ($this->settings['options'] && $data)
		{
			...
			$tagdata = ee()->TMPL->swap_var_single('total_selections', (string)count($data), $tagdata);
			...
			foreach($data as $option_name)
			{
				if (($option = $this->_find_option($option_name, $this->settings['options'])) !== FALSE)
				{
					...
					// simple var swaps
					$option_tagdata = ee()->TMPL->swap_var_single('option', $option, $option_tagdata);
					$option_tagdata = ee()->TMPL->swap_var_single('option_name', $option_name, $option_tagdata);

					// seems there was a mix-up in the documentation and things where ambiguous. Let's make it crystal clear.
					$option_tagdata = ee()->TMPL->swap_var_single('option_value', $option_name, $option_tagdata);
					$option_tagdata = ee()->TMPL->swap_var_single('option_label', $option, $option_tagdata);
	...
	function replace_element_tag($data, $params = array(), $tagdata)
	{
		...
		return ee()->functions->var_swap($tagdata,
			array(
