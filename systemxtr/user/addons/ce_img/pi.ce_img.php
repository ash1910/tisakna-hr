...
====================================================================================================
 ...
 copyright 		Copyright (c) 2016 Causing Effect, Aaron Waldon <aaron@causingeffect.com>
----------------------------------------------------------------------------------------------------
...
class Ce_img
{
	...
	function __construct()
	{
		...
		{
			self::$started = true;
			if (ee()->extensions->active_hook('ce_img_start'))
			{
				ee()->extensions->call('ce_img_start');
			}
		}

		//if the template debugger is enabled, and a super admin user is logged in, enable debug mode
		$this->debug = ( ee()->session->userdata['group_id'] == 1 );
	}
	...
	private function get_open_defaults()
	{
		...
		//fallback_src param
		$defaults['fallback_src'] = ee()->TMPL->fetch_param('fallback_src', '');

		//made_regex - check global array and config
		if ( isset( ee()->config->_global_vars['ce_image_made_regex'] ) && ee()->config->_global_vars['ce_image_made_regex'] != '' )
		{
			//since the EE team unexpectedly botched the ability to have arrays as global vars, we now have to work with a string...
			$temp = ee()->config->_global_vars['ce_image_made_regex'];

			$defaults['made_regex'] = $this->ensure_array( $temp );
		}
		else //config arrays still work...
		{
			$defaults['made_regex'] = ee()->config->item('ce_image_made_regex');
		}
		...
		if ( isset( ee()->config->_global_vars['ce_image_src_regex'] ) && ee()->config->_global_vars['ce_image_src_regex'] != '' )
		{
			//since the EE team unexpectedly botched the ability to have arrays as global vars, we now have to work with a string...
			$temp = ee()->config->_global_vars['ce_image_src_regex'];

			$defaults['src_regex'] = $this->ensure_array( $temp );
		}
		else //config arrays still work...
		{
			$defaults['src_regex'] = ee()->config->item('ce_image_src_regex');
		}
	...
	public function get_make_defaults()
	{
		...
		$defaults['allow_overwrite_original'] = ( strtolower(ee()->TMPL->fetch_param('allow_overwrite_original')) == 'yes' );

		//allow_scale_larger param
		$defaults['allow_scale_larger'] = ( strtolower(ee()->TMPL->fetch_param('allow_scale_larger')) == 'yes' );

		//bg_color param
		$defaults['bg_color'] = ee()->TMPL->fetch_param('bg_color');

		//bg_color_default param
		$defaults['bg_color_default'] = ee()->TMPL->fetch_param('bg_color_default', 'ffffff');

		//border param
		$border = trim( ee()->TMPL->fetch_param('border') );
		if ( $border != '' )
		{
		...
		//crop param
		$crop = strtolower( ee()->TMPL->fetch_param('crop') );
		...
		//filename param
		$defaults['filename'] = trim( ee()->TMPL->fetch_param('filename', '') );

		//filename_prefix param
		$defaults['filename_prefix'] = trim( ee()->TMPL->fetch_param('filename_prefix', '') );

		//filename param
		$defaults['filename_suffix'] = trim( ee()->TMPL->fetch_param('filename_suffix', '') );

		//filter param
		$filter = trim( ee()->TMPL->fetch_param('filter', '') );
		$filters = array();
		if ( ! empty( $filter ) )
		{
		...
		//hash_filename param
		$defaults['hash_filename'] = ( strtolower(ee()->TMPL->fetch_param('hash_filename')) == 'yes' );

		//force_remote param
		$defaults['force_remote'] = $this->ee_string_to_bool( $this->determine_setting( 'force_remote', 'n', false ) );

		//flip param
		$flip = strtolower( ee()->TMPL->fetch_param('flip') );
		if ( $flip == '' )
		{
			$defaults['flip'] = FALSE;
		}
		...
		$defaults['hide_relative_path'] = ( ee()->TMPL->fetch_param('hide_relative_path') == 'yes' );

		//height param
		$defaults['height'] = ee()->TMPL->fetch_param('height', 0);
		//width param
		$defaults['width'] = ee()->TMPL->fetch_param('width', 0);

		//max param
		$max = ee()->TMPL->fetch_param('max', 0);
		//max_height param
		$defaults['max_height'] = ee()->TMPL->fetch_param('max_height', $max);
		//max_width param
		$defaults['max_width'] = ee()->TMPL->fetch_param('max_width', $max);

		//min param
		$min = ee()->TMPL->fetch_param('min', 0);
		//min_height param
		$defaults['min_height'] = ee()->TMPL->fetch_param('min_height', $min);
		//min_width param
		$defaults['min_width'] = ee()->TMPL->fetch_param('min_width', $min);

		//overwrite_cache param
		$defaults['overwrite_cache'] = ( strtolower(ee()->TMPL->fetch_param('overwrite_cache')) == 'yes' );

		//image permissions
		if ( ee()->config->item('ce_image_image_permissions') != FALSE && is_numeric( ee()->config->item('ce_image_image_permissions') ) )
		{
			$this->image_permissions = ee()->config->item('ce_image_image_permissions');
		}
		$defaults['image_permissions'] = $this->image_permissions;

		//directory permissions
		if ( ee()->config->item('ce_image_dir_permissions') != FALSE && is_numeric( ee()->config->item('ce_image_dir_permissions') ) )
		{
			$this->dir_permissions = ee()->config->item('ce_image_dir_permissions');
		}
		...
		//reflection param
		$reflection = trim( ee()->TMPL->fetch_param('reflection') );
		...
		//rotate param
		$defaults['rotate'] = strtolower( ee()->TMPL->fetch_param('rotate') );

		//rounded_corners param
		$corners = trim( ee()->TMPL->fetch_param('rounded_corners') );
		if ( $corners != '' )
		{
		...
		//save_type param
		$defaults['save_type'] = ee()->TMPL->fetch_param('save_type');
		...
		//watermark param
		$watermark = ee()->TMPL->fetch_param('watermark');
		if ( $watermark != '' )
		{
	...
	protected function log_debug_messages( $messages = array() )
	{
		if ( $this->debug )
		{
			...
			{
				ee()->TMPL->log_item( '&nbsp;&nbsp;***&nbsp;&nbsp;CE Image debug: ' . $message );
			}
		}
	}
	...
	{
		//manipulate param
		$manipulate = ( strtolower(ee()->TMPL->fetch_param('manipulate') ) == 'no' ) ? FALSE : TRUE;
		...
		if ( $src == '' && $defaults['fallback_src'] == '' )
		{
			if ( $this->debug )
			{
				ee()->TMPL->log_item( '&nbsp;&nbsp;***&nbsp;&nbsp;CE Image debug: Source and fallback source cannot both be blank.' );
			}
		...
		//ascii_art param
		$ascii_art = trim( ee()->TMPL->fetch_param('ascii_art') );
		if ( $ascii_art != '' )
		{
		...
		//top_colors param
		$top_colors = ee()->TMPL->fetch_param('top_colors');
		...
		$create_tag = ( strtolower(ee()->TMPL->fetch_param('create_tag')) == 'yes' );

		//image tag attributes
		$attributes = array();

		if ( ee()->TMPL->fetch_param('url_only') == 'yes' )
		{
			$tagdata = '{' . $var_prefix . 'made}';
			$create_tag = FALSE;
		}
		else if (ee()->TMPL->fetch_param('output') != '')
		{
			$tagdata = ee()->TMPL->fetch_param('output');
			$create_tag = FALSE;
		...
		else
		{
			...
			else if (ee()->TMPL->fetch_param('create_tag') == '' && $tagdata == '') //if there is no tag data, set create_tag to TRUE
			{
				$create_tag = TRUE;
			}
		...
		$variables = array(
			...
			$var_prefix . 'made_url' 			=> ee()->functions->create_url( $relative ),
			$var_prefix . 'orig_url' 			=> ee()->functions->create_url( $relative_orig ),
		...
		//pre parse hook
		if (ee()->extensions->active_hook('ce_img_pre_parse'))
		{
			$tagdata = ee()->extensions->call('ce_img_pre_parse', $tagdata, $variables, $var_prefix );
		}

		//parse
		$parsed = ee()->TMPL->parse_variables_row( $tagdata, $variables );
	...
	private function parse_ee_paths( $string )
	{
		...
		{
			$string = str_replace( '{site_url}', stripslashes( ee()->config->item( 'site_url' ) ), $string );
		}

		//replace the path= variables if applicable
		if ( strpos( $string, 'path=' ) !== FALSE )
		{
			$string = preg_replace_callback( '@{\s*path=(.*?)}@', array( &ee()->functions, 'create_url' ), $string );
		}

		//replace the file directory paths if applicable
		if ( strpos( $string, 'filedir_' ) !== FALSE)
		{
			$filedirs = ee()->functions->fetch_file_paths();
	...
	public function bulk()
	{
		...
		if ( ee()->TMPL->fetch_param('exclude_regex') !== FALSE )
		{
			$exclude = ee()->TMPL->fetch_param('exclude_regex');
		}
		else if ( isset( ee()->config->_global_vars['ce_image_exclude_regex'] ) && ee()->config->_global_vars['ce_image_exclude_regex'] != '' )
		{
			//since the EE team unexpectedly botched the ability to have arrays as global vars, we now have to work with a string...
			$exclude = ee()->config->_global_vars['ce_image_exclude_regex'];
		}
		else //config arrays still work...
		{
			$exclude = ee()->config->item('ce_image_exclude_regex');
		}
		if ( is_bool( $exclude ) )
		{
			$exclude = array();
		...
		foreach ( $matches as $match )
		{
			...
			if ( ee()->TMPL->fetch_param('dimensions_from_style') != 'no' )
			{
	...
	protected function base64_image( $path, $type )
	{
		...
		if ( ! $contents )
		{
			ee()->TMPL->log_item( '&nbsp;&nbsp;***&nbsp;&nbsp;CE Image debug: There was an error getting the contents of the file "' . $path . '" to convert to base64.' );
		}
		else
		{
			...
			if ( empty( $contents ) )
			{
				ee()->TMPL->log_item( '&nbsp;&nbsp;***&nbsp;&nbsp;CE Image debug: There was an error converting the contents of the file "' . $path . '" to base64.' );
			}
			else
			{
	...
	protected function get_override_attributes( $tag_attributes = array() )
	{
		...
		$attributes = ee()->TMPL->fetch_param('attributes');
