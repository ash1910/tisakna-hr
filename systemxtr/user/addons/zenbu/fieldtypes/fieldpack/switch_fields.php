...
class Fieldpack_switch_fields extends Base
{
	...
	*/
	function zenbu_display($data, $field_id)
	{
		...
		$output = $this->display->text($output, $field_id, $settings);
					...
					case "contains" :
						if(empty($keyword_in_db))
						{
							if(empty($keyword))
							{
								...
								$like_query = 'field_id_'.$field_id.' LIKE "%'.ee()->db->escape_like_str($keyword).'%"';
							}
						} else {
							$like_query = 'field_id_'.$field_id.' LIKE "%'.ee()->db->escape_like_str($keyword_in_db).'%"';
						}
					break;
					case "doesnotcontain" :
						if(empty($keyword_in_db))
						{
							...
							$like_query = 'field_id_'.$field_id.' NOT LIKE "%'.ee()->db->escape_like_str($keyword_in_db).'%" OR field_id_'.$field_id.' IS NULL';
						}
					break;
				}
			}
