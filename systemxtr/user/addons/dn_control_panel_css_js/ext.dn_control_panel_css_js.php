...
class Dn_control_panel_css_js_ext
{
	...
	/**
	 * Due to a bug (#21601) in versions of EE 3.x prior to 3.2.0, EE saves its extension settings
	 * only to the first method called from our list of hooks (cp_css_end), so subsequent methods have empty settings
	 *
	 * For this reason, we need to check the version of EE and see whether the settings are empty.
	 * If so, we need to get the extension settings saved against the first method in the database.
	 * 
	 * @return	void
	 */
	private function load_settings()
	{
		if (empty($this->settings) && version_compare(APP_VER, '3', '>') && version_compare(APP_VER, '3.2', '<'))
		{
