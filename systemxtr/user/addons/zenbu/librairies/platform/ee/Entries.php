...
class Entries extends Base
{
	...
	public function getEntries()
	{
		...
		*/
		...
		if( ! empty($channel_id) )
		{
			...
			$section_array = ArrayHelper::make_array_of('channel_id', $this->getSections());
		...
		//	----------------------------------------
		//	If no filter POST values are found
		//	Try getting filters from searchId, and
		//	permute array to work with following
		//	conditional in this method.
		//	----------------------------------------
		if(! $filters)
		{
			$filters = $this->saved_searches->getSavedSearchFilters();

			if($filters)
			{
				$filters = $this->saved_searches->permuteSavedSearchFilters($filters);
			}
		}
				...
				if(isset($filter['1st']))
				{
					switch ($filter['1st'])
					{
						...
						case 'status':
							 ...
							 */
							$status = isset($filter['3rd']) ? $filter['3rd'] : '';
						...
						case 'author':
							 ...
							 */
							$author_id = isset($filter['3rd']) ? $filter['3rd'] : '';
						...
						case is_numeric($filter['1st']) :
							...
							$field_id = $filter['1st'];
							$where = "";
							$this->field_ids = ArrayHelper::make_array_of('field_id', $this->getAllFields());
							...
							if(in_array($field_id, $this->field_ids))
							{
								if(isset($this->fieldtypes[$field_id]))
								{
									...
									*/
									...
									$ft_object = $this->fieldsObj->loadFieldtypeClass($this->fieldtypes[$field_id]);
		...
		switch ($orderby)
		{
			...
			case is_numeric($orderby):
				// If no custom fields are set to be displayed
				// but you're still ordering by a custom field
				// JOIN the exp_channel_data table since it
				// wasn't JOINed yet
				if(! isset($has_custom_fields))
				{
					ee()->db->join('channel_data', 'exp_channel_titles.entry_id = exp_channel_data.entry_id');
	...
	{
		$fields = ArrayHelper::flatten_to_key_val('field_id', 'field_type', $this->getAllFields());
		$field_ids = ArrayHelper::make_array_of('field_id', $this->getAllFields());
		...
		foreach($entries as $entry_key => $entry)
		{
			foreach($entry as $key => $val)
			{
				if(strncmp($key, 'field_id_', 9) == 0)
				{
					...
					$fieldtype = $fields[$field_id];

					// Get a basic list of rel data
					foreach($fields as $fid => $ftype)
					{
					...
					*/
					...
					$ft_object = $this->fieldsObj->loadFieldtypeClass($this->getFieldtypes()[$field_id]);
					...
					$$ft_data 	= $ft_object && method_exists($ft_object, 'zenbu_get_table_data') ? $ft_object->zenbu_get_table_data(
							...
							$field_ids,
							$entry['channel_id'],
							$this->files->upload_preferences(), // $output_upload_prefs,
							array(),//$settings,
							$rel_array) : array();
					...
					$output[$entry_key][$key]	= $field_data = $ft_object && method_exists($ft_object, 'zenbu_display') ?
					...
					*/
					if (ee()->extensions->active_hook('zenbu_modify_field_cell_data') === TRUE)
					{
						$info_data						= array_merge($field_ids, $entry);
						$info_data['current_field_id']	= $field_id;
						$info_data['fieldtype'][$field_id] = $fieldtype;

						$output[$entry_key][$key] = ee()->extensions->call('zenbu_modify_field_cell_data', $field_data, $info_data);
					 	if (ee()->extensions->end_script === TRUE) return;
					}
				}
