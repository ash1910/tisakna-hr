...
class Zenbu_tagger_ft extends Tagger_ft
{
	...
	*/
	public function zenbu_display($entry_id, $channel_id, $data, $tagger_data = array(), $field_id, $settings, $rules = array())
	{
		...
		$output = ee()->zenbu_display->highlight($output, $rules, 'field_'.$field_id);
		return $output;
	}
	...
	*/
	public function zenbu_get_table_data($entry_ids, $field_ids, $channel_id)
	{
		...
		ee()->db->select("exp_tagger.tag_name");
		ee()->db->select("exp_tagger_links.entry_id");
		ee()->db->join("exp_tagger_links", "exp_tagger.tag_id = exp_tagger_links.tag_id");
		foreach($entry_ids as $key => $entry_id)
		{
			ee()->db->or_where("exp_tagger_links.entry_id", $entry_id);
		}
		$query = ee()->db->get("exp_tagger");
	...
	public function zenbu_result_query($rules = array())
	{
		...
		foreach($rules as $rule)
		{
			...
			if(strncmp($rule['field'], 'field_', 6) == 0 && ! empty($keyword))
			{
				...
				{
					$query_not_like = ee()->db->query("SELECT exp_tagger_links.entry_id FROM exp_tagger_links JOIN exp_tagger ON exp_tagger_links.tag_id = exp_tagger.tag_id WHERE exp_tagger.tag_name NOT LIKE '%".ee()->db->escape_like_str($keyword)."%' GROUP BY exp_tagger_links.entry_id");
					$query_like = ee()->db->query("SELECT exp_tagger_links.entry_id FROM exp_tagger_links JOIN exp_tagger ON exp_tagger_links.tag_id = exp_tagger.tag_id WHERE exp_tagger.tag_name LIKE '%".ee()->db->escape_like_str($keyword)."%' GROUP BY exp_tagger_links.entry_id");
				...
				if($query_like->num_rows() > 0)
				{
					...
					if($in == "doesnotcontain")
					{
						ee()->db->where_not_in("exp_channel_titles.entry_id", $entries);
					} else {
