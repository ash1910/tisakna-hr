...
abstract class AbstractTag
{
    ...
    {
        ee()->TMPL->log_item('STORE: ' . $msg);
    }

    public function parse_variables(array $tag_vars)
    {
        return ee()->TMPL->parse_variables($this->tagdata, $tag_vars);
    }
    ...
    protected function tmpl_secure_check($use_global = true)
    {
        ...
        {
            if (ee()->store->store->secure_request()) {
                ...
                ee()->functions->redirect(str_replace('http://', 'https://', ee()->functions->fetch_current_uri()));
            }
        }
    }
    ...
    protected function form_open($action, $hidden_fields = array(), $extra_html = array())
    {
        ...
        $hidden_fields['ACT'] = ee()->functions->fetch_action_id('Store', $action);
        $hidden_fields['RET'] = ee()->uri->uri_string;
        ...
        $hidden_fields['_params'] = ee()->encrypt->encode(json_encode($this->params));
    ...
    {
        $theme_url = ee()->store->config->asset_url('store.js');
        $theme_url = str_ireplace('http://', '//', $theme_url);

        return '
            (function() {
                ExpressoStore.config = '.ee()->store->config->config_json().';
    ...
    protected function get_orders_query()
    {
        ...
        if ('yes' == $paid) {
            $query->where('order_paid', '>=', ee()->store->db->raw('`order_total`'));
        } elseif ('no' == $paid) {
            $query->where('order_paid', '<', ee()->store->db->raw('`order_total`'));
        }
        ...
        if (empty($orderBy) || $orderBy == 'order_id') {
            ...
            $query->orderBy(ee()->store->db->raw('RAND()'));
        } else {
    ...
    {
        $order_hash = ee()->input->cookie('store_cart_submit');
        ...
        if ($conversion_tracking_extra) {
            $out .= ee()->TMPL->parse_variables($conversion_tracking_extra, array($order));
        }

        // conversion tracking should only ever happen once per order, unset cookie
        //ee()->input->delete_cookie('store_cart_submit');
		ee()->input->delete_cookie('store_cart_submit');
        return $out;
    }
    ...
    protected function add_payment_tag_vars(&$tag_vars)
    {
        ...
        if (($payment_error = ee()->session->flashdata('store_payment_error')) !== false) {
        ...
        if (strpos($this->tagdata, '{payment_method_options}') !== false) {
            ...
            $tag_vars[0]['payment_method_options'] = ee()->store->payments->get_enabled_payment_method_options($payment_method);
        }
        ...
        if (preg_match_all('/\{(\w+):issuer_options\}/', $this->tagdata, $issuer_matches)) {
            foreach ($issuer_matches[1] as $gateway) {
                $tag_vars[0][$gateway.':issuer_options'] = ee()->store->payments->fetch_issuer_options($gateway);
            }
        }
    }
