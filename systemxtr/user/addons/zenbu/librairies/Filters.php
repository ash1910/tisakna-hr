...
class Filters extends Base
{
    ...
    public function getFirstFilterOptions()
    {
        ...
        unset($output[0][0]['livelook']);

        foreach($this->getSections() as $key => $section)
        {
            $subsections = $this->sectionsObj->getSubSections($section->id);

            if($subsections)
            {
                foreach($subsections as $subsection)
                {
                    $filter_fields = $this->fieldsObj->getFields($section->id, $subsection->id);
                    unset($filter_fields[Convert::string('section')]); // Don't need this, we have it as the top dropdown
                    unset($filter_fields['livelook']);
            ...
            {
                $filter_fields = $this->fieldsObj->getFields($section->id, 0);
                unset($filter_fields[Convert::string('section')]); // Don't need this, we have it as the top dropdown
                unset($filter_fields['livelook']);
    ...
    public function getLimitSelectOptions()
    {
        ...
        //    ----------------------------------------
        //    Add general setting for
        //    Max results per page, when available
        //    ----------------------------------------
        if(isset($this->general_settings['max_results_per_page']) && ! empty($this->general_settings['max_results_per_page']))
        {
            $output[$this->general_settings['max_results_per_page']] = Lang::t('Show') . ' ' . $this->general_settings['max_results_per_page'] . ' ' . Lang::t('results');
            ksort($output);
        }
    ...
    {
        foreach($this->fieldsBase->std_fields as $handle => $field)
        {
            $output[0][$field['handle']] = $field['name'];
        }

        unset($output[0]['livelook']);

        foreach($this->sections as $key => $section)
        {
            $this->sectionsObj->setSubSections($section->id);
            $subsections = $this->sectionsObj->getSubSections(null);
    ...
    public function cacheFilters()
    {
        ...
        Cache::set('zenbu_filter_cache', $searchfilter, 600);
