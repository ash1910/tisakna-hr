...
class Zenbu_PermissionsController extends ZenbuBaseController
{
    ...
    public function actionIndex()
    {
		...
		$this->cp->initSidebar();
		$this->vars['permissions']    = $this->settingsObj->getPermissionsAllGroups();
		...
		$this->vars['module_access']  = $this->settingsObj->getGroupsWithAddonAccess();
		...
		View::includeJs(array(
			'resources/js/zenbu_permissions.js',
			));
		return array(
    ...
    public function actionSave()
    {
		...
		foreach($members as $group_id => $perm_array)
		{
			foreach($perm_array as $perm => $value)
			{
				...
				$permissions_settings->value       = $group_id == 1 && $perm != 'edit_replace' ? 'y' : $value;
				$permissions_settings->save();
			}
			$this->cache->delete('permissions', 'group_'.$group_id);
		}
		...
		$this->cache->delete('permissions', 'all_groups');
