...
class Statuses extends Base
{
	...
	function getStatuses()
	{
		...
		/* Do the query */
		$results = ee()->db->query("/* Zenbu _get_statuses */ \n SELECT s.* FROM exp_statuses s WHERE s.site_id = ".$this->user->site_id." GROUP BY s.status, s.status_id");

		if($results->num_rows() > 0)
		{
			$output = $results->result_array();
			$this->cache->set('statuses', $output, 300);
			$results->free_result();
			return $output;
		} else {
	...
	function getStatusByChannel()
	{
		...
		if($results->num_rows() > 0)
		{
			...
			// $output = $results->result_array();
			$this->cache->set('statuses_by_channel', $output, 300);
			$results->free_result();
			return $output;
		}
	}
	...
	{
		return array('' => Lang::t('any_status')) + ArrayHelper::flatten_to_key_val('status', 'status', $this->getStatuses());
	}
}
