...
class Datagrab_model extends CI_Model {
	...
	function do_import( $datatype, $settings ) {
		...
		*/
		...
		// Loop over items
		$dataItems = array();
		if($this->settings["import"]["type"] == 'ajw_sql')
			$dataItems = $datatype->items;
		else
		...
		foreach ($dataItems as $item) {
			...
			// Get URL title
			$search = array("Š", "Đ", "Č", "Ć", "Ž", "š", "đ", "č", "ć", "ž");
			$replace = array("s", "d", "c", "c", "z", "s", "d", "c", "c", "z");
			$titleSR = str_replace( $search, $replace, $data["title"]);

			//$data["url_title"] = url_title( strtolower( $titleSR ) );
			$data["url_title"] = $this->url_title_new( strtolower( $titleSR ) );

			$option = [];
			$option["url_title"] = $data["url_title"];
			$option["channel_id"] = $this->channel_defaults["channel_id"];
			$option["entry_id"] = 0;
			$option = $this->check_url_title($option);
			$url_title_new_with_increment = isset($option["url_title"]) ? $option["url_title"] : "";
			...
			foreach( $custom_fields as $field => $field_data ) {
				...
				if( isset( $this->settings["cf"][ $field ] ) && $this->settings["cf"][ $field ] != "" ) {
					...
					if ( class_exists('Datagrab_'.$field_data[ "type" ]) ) {
						...
						if( ($entry_id == 0) || ($entry_id > 0 && isset( $this->settings["config"]["update"] ) && ($this->settings["config"]["update"] == "y")) )
			...
			if (  $entry_id == 0 ) {
				...
				// check duplicate url_title
				if( isset($url_title_new_with_increment) && !empty($url_title_new_with_increment) ){
					$_POST["url_title"] = $data["url_title"] = $url_title_new_with_increment;
				} 
				...
				$channel = ee('Model')->get('Channel', $data["channel_id"])
							->first();
				...
				if( $result->isValid() ) {
					...
					foreach( $errors as $err_field => $error ) {
						foreach( $error as $err_rule => $err_message ) {
							$this->return_data .= "<p>Error in entry: " . $data["title"] . ", field: " . $err_field . ", message: " . lang($err_message) . "</p>\n";
						}
					}
				...
				if( isset( $this->settings["config"]["update"] ) && $this->settings["config"]["update"] == "y") {
					...
					if( $query->num_rows() ) {
						$row = $query->row_array();
						$_POST["url_title"] = $data["url_title"] = $row["url_title"];
					}
				...
				$data["existing_entry"] = 1; // for detecting existing entry
	...
	*/
	...
	function _create_category( $category_name, $category_group="", $site_id=1, $parent_id=0 ) {
		...
		if ( $query->num_rows == 0) {
			...
			$searchcat = array("Š", "Đ", "Č", "Ć", "Ž", "š", "đ", "č", "ć", "ž");
			$replacecat = array("s", "d", "c", "c", "z", "s", "d", "c", "c", "z");
			$category_nameSR = str_replace( $searchcat, $replacecat, $category_name);
			$custom_url_title = url_title( $category_nameSR, $this->config->item('word_separator'), TRUE );

			$slug = $custom_url_title;
			$i = 1; $baseSlug = $slug;
			while( $this->slug_exist($slug) ){
			    $slug = $baseSlug . "-" . $i++;        
			}
			$custom_url_title = $slug;
			
			$insert_array = array(
				'group_id' => $category_group,
				'site_id' => $site_id,
				'cat_name' => $category_name,
				'cat_url_title' => $custom_url_title,
				'cat_image' => '',
				'parent_id' => $parent_id,
				'cat_order' => 1
				);
	...
	*/
	...
	function url_title_new($str, $separator = 'dash', $lowercase = FALSE)
	{
		if (UTF8_ENABLED)
		{
			ee()->load->helper('text');
...
public function slug_exist($slug){
	...
	$querycat = $this->db->get_where('exp_categories', array("cat_url_title" => $slug))->row();
	//echo "<pre>";print_r($querycat);exit;
	if( isset($querycat) && !empty($querycat) ){
		return true;
	}
	return false;
}
