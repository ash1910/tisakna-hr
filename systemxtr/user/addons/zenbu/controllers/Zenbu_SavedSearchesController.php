...
class Zenbu_SavedSearchesController extends ZenbuBaseController
{
    ...
    {
        parent::init();
        $this->default_limit = isset($this->general_settings['default_limit']) && is_numeric($this->general_settings['default_limit']) ? $this->general_settings['default_limit'] : 25;
        $this->default_sort  = isset($this->general_settings['default_sort']) && in_array($this->general_settings['default_sort'], array('ASC', 'DESC')) ? $this->general_settings['default_sort'] : 'ASC';
        $this->default_order = isset($this->general_settings['default_order']) && ! empty($this->general_settings['default_order']) ? $this->general_settings['default_order'] : 'title';
    }
    ...
    public function actionSave()
    {
        ...
        {
            $found = Db::find('zenbu_saved_searches', 'id IN(?)', array(
            ...
            if(count($found) > 0)
            {
                ...
                {
                    // Find the filters
                    $found_filters = Db::find('zenbu_saved_search_filters', 'searchId = ?', array($found->id), array('order', 'asc'));

                    foreach($search_copy as $group_id)
                    {
                        $current_group_searches = Db::find('zenbu_saved_searches', 'userGroupId = ?', array(
                                $group_id
                            ), array('order', 'asc')
                        );
                        $saved_search              = new Model\ZenbuSavedSearchesModel();
    ...
    public function actionCreate()
    {
        ...
        $searchfilter->filterAttribute2 = Request::param('limit', $this->default_limit);
        ...
        $searchfilter->filterAttribute2 = Request::param('orderby', $this->default_order);
        $searchfilter->filterAttribute3 = Request::param('sort', $this->default_sort);
        $searchfilter->order            = $c;
        $searchfilter->save();
        //$this->db->insert('zenbu_saved_search_filters', $searchfilter);
    ...
    public function actionCacheTempFilters()
    {
        ...
        $searchfilter[$c]['filterAttribute2'] = Request::param('limit', $this->default_limit);
        $searchfilter[$c]['filterAttribute3'] = '';
        $searchfilter[$c]['order']            = $c;
        $c++;

        $searchfilter[$c]['filterAttribute1'] = 'orderby';
        $searchfilter[$c]['filterAttribute2'] = Request::param('orderby', $this->default_order);
        $searchfilter[$c]['filterAttribute3'] = Request::param('sort', $this->default_sort);
        $searchfilter[$c]['order']            = $c;

        Cache::set('TempFilters', $searchfilter, 60);
    ...
    {
        if(Cache::get('TempFilters') !== FALSE)
        {
            return json_encode(Cache::get('TempFilters'));
        }
        else
        {
