<?php
	...
	$this->table->set_columns(array(
		...
		'field' => array('header' => 'Field')		
	));
    $this->table->add_row(
        $form->label('name', null, array('required' => true)),
        $form->input('name').form_error('sale[name]')		
    );
    ...
    $this->table->add_row(
        ...
        $form->select('entry_ids', $product_options, array('multiple' => true, 'limit' => 50))
    );
