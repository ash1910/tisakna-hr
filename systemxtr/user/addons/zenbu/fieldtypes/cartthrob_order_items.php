...
class Zenbu_cartthrob_order_items_ft extends Base
{
	...
	*/
	public function zenbu_display($entry_id, $channel_id, $data, $table_data = array(), $field_id, $settings, $filters = array(), $upload_prefs = array(), $installed_addons)
	{
		...
		{
			// Get headers
			$header_array_original = array();
			$header_array_original_nonextra = array();
			foreach($table_data['order_id_'.$entry_id] as $row_id => $data_array)
			{
				...
				foreach($data_array as $label => $val)
				{
					if($label == 'extra' && is_array($data_array['extra']))
					{
						...
						{
							if($h != "row_id" && ! in_array($h, $header_array_original))
							{
								$header_array[] = ucwords(str_replace('_', ' ', $h));
								$header_array_original[] = $h;
							}
						}
					}
					elseif(! in_array($label, $header_array_original_nonextra))
					{
						$header_array[] = ucwords(str_replace('_', ' ', $label));
						$header_array_original_nonextra[] = $label;
					}
				}
			}
			...
			foreach($table_data['order_id_'.$entry_id] as $row_id => $data_array)
			{
				...
				foreach($data_array as $key => $val)
				{
					...
					{
						$extra = $val;
						foreach($header_array_original as $header_key => $header_value)
						{
							if(isset($extra[$header_value]))
							{
								if( ! empty($extra[$header_value]) && ! is_array($extra[$header_value]))
								{
									$data_array_output[] = $extra[$header_value];
								}
								elseif( ! empty($extra[$header_value]) && is_array($extra[$header_value]) )
								{
									$voutput = '<ul>';
									foreach($extra[$header_value] as $vkey => $vval)
									{
										$voutput .= '<li>';
										if(is_array($vval))
										{
