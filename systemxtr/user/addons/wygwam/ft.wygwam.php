...
class Wygwam_ft extends EE_Fieldtype {
	...
	// TODO: Remove when EE actually starts using addon.setup.php
	public $info = array(
		'name' => 'Wygwam',
		'version' => '4.1.3'
	);
	...
	public function display_field($data)
	{
		...
		return '<div class="wygwam"><textarea id="'.$id.'" name="'.$this->field_name.'" rows="10" data-config="'.$configHandle.'" data-defer="'.(!empty($this->settings['defer']) && $this->settings['defer'] == 'y' ? 'y' : 'n').'">'.htmlspecialchars($data).'</textarea></div>'.$this->_generateAssetInputsString($assetInfo);
	}
	...
	public function grid_display_field($data)
	{
		...
		return '<textarea name="'.$this->field_name.'" rows="10">'.$data.'</textarea>'.$this->_generateAssetInputsString($assetInfo);
	}
	...
	public function display_cell($data)
	{
		...
		return '<textarea name="'.$this->cell_name.'" rows="10">'.$data.'</textarea>'.$this->_generateAssetInputsString($assetInfo);
	}
	...
	{
		return in_array($name, array('channel', 'grid', 'low_variables', 'fluid_field'));
	}
	...
	{
		$inputString = '';
		for ($counter = 0; $counter < count($assetInfo['ids']); $counter++)
		{
