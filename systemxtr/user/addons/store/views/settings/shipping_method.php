...
<?php
    ...
    $this->table->add_row(
        lang('name', 'method[name]').'<strong class="notice">*</strong>' ,
		form_input('ship_method[name]',$shipping_method->name)
		.form_error('ship_method[name]')
    );
    $this->table->add_row(
        lang('store.enabled', 'method[enabled]'),
		store_form_checkbox('ship_method[enabled]',$shipping_method->enabled)        
    );
...
<?php if ($shipping_method->exists): ?>
    ...
    <?php
        ...
        $this->table->set_heading(
            ...
            array('data' => lang('store.shipping_charges'), 'width' => '10%'),
            array('data' => lang('store.enabled'), 'width' => '15%'),
