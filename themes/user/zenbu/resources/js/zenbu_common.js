...
{
    // if ($("div.zenbu-filters").find("select").data('select2')) {
    //     $("div.zenbu-filters").find("select").select2('destroy');
    // }
    $("div.zenbu-filters").find("select").select2({search_contains: true, theme: 'ee3'});
}
...
$(document).ready(function (){
    ...
    $("body").delegate('table.mainTable th input[type=checkbox], table.settingsTable th input[type=checkbox]', 'click', function (e) {
        ...
        $('table.mainTable tr, table.settingsTable tr').each(function() {
            $(this).find('td').eq(columnIndex).find('input[type=checkbox]:not(:disabled)').prop('checked', checked);
        });
    });
