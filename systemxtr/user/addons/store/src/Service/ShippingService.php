...
class ShippingService extends AbstractService
{
        ...
        if ($this->ee->extensions->active_hook('store_order_shipping_methods')) {
            try {
                ...
                if ($this->ee->input->is_ajax_request() && $this->ee->config->item('store_new_json_response') == 'yes') {
                    $this->returnJsonResponse($order, $e->getMessage());
                }

                $this->ee->output->show_user_error(false, array("Store (Shipping): ".$e->getMessage()));
            }
        }
    ...
    private function returnJsonResponse($cart, $error)
    {
        $out = array();
        $out['cart'] = $cart->toTagArray();
        $out['form_errors'] = array('shipping' => $error);
