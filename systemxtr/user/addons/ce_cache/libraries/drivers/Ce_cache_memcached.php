...
class Ce_cache_memcached extends Ce_cache_driver
{
	...
	private function setup()
	{
		if ( ! $this->is_setup ) //if not already setup
		{
			...
			if ( ! empty( ee()->config->_global_vars[ 'ce_cache_memcached_servers' ] ) ) //first check the global array
			{
				$config = ee()->config->_global_vars[ 'ce_cache_memcached_servers' ];
			}
			else if ( ee()->config->item( 'ce_cache_memcached_servers' ) != false ) //then check the config
			{
				$config = ee()->config->item( 'ce_cache_memcached_servers' );
			}
