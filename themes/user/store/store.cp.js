...
(function() {
    ...
    StoreMultiSelect = (function() {
        ...
        StoreMultiSelect.prototype.refresh_old = function() {
            var query,
            _this = this;
        ...
        StoreMultiSelect.prototype.refresh = function() {
            ...
            _this = this;
            query = this.filter.val().toLowerCase();
			var limit = _this.select.attr('limit');
			var id = _this.select.attr('id');
			var leftindex = 0;
			
			if(limit && (id == "sale_entry_ids" || id == "discount_entry_ids" )){
				
				$.ajax({
				  url: "index.php?/cp/addons/settings/store&sc=sales&sm=get_product_list",
				  type: "POST",
				  data: { limit : limit, q : query },
				  ...
				  .done(function( data ) {
						...
						$.each($.parseJSON(data), function(index, item) {
							...
							var text = item.title;
							var element = $('<li/>').attr('data-id', id).text(text);
							
							if ((_this.select.find("option[value='" + id + "']")).attr('selected')) {
								return "";
							} else {
								return _this.list1.append(element);
							}
							
						});
						
						return $.each(_this.select.find('option:selected'), function(index, option){

							var item = $('<li/>').attr('data-id', $(option).val()).text($(option).text());
							return _this.list2.append(item);
				...
				return $.each(this.items, function(index, item) {
					...
					if( limit && limit <= leftindex ) return false;
    ...
    $(function() {
        ...
        if ($.datepicker != null) {
            (function() {
				...
				$('input.store_date').datepicker({
                    dateFormat: $.datepicker.W3C
                });				
                dateTimeElems = $('input.store_datetime');
		...
		$('#store_datatable_search select[name="per_page"]').change(function(){			
			$('#store_datatable_search').submit();
		});
		$('#store_datatable_search select[name="category_id"]').change(function(){			
			$('#store_datatable_search').submit();
		});
		...
		$('#country_edit').submit(function(e){
			var names = [];
			var flag = true;
			$("input[type='text']").each(function(){
				...
				if($(this).attr('name').indexOf('code') != -1 && $(this).val().length > 5){
					alert("Code should not be more than 5 letters");
					flag=false;
					return false;		
				}
			});	
        ...
        //if ($('#status_color').size()) {
            ...
            $('.store_colorswatch').click(function() {				
        ...
        if ($("#store_product_field").size() > 0) {
            return (function() {
                ...
                $("#store_product_modifiers_add").click(function() {
					...
					mod_key = ((_ref = $('.ajax-validate').serializeObject().store_product_field) != null ? (_ref1 = _ref.modifiers) != null ? _ref1.length : void 0 : void 0) || 0;
                ...
                modifiersTable.on("click", "a.store_product_option_add", function() {
                    ...
                    opt_key = ((_ref = $('.ajax-validate').serializeObject().store_product_field.modifiers[mod_key].options) != null ? _ref.length : void 0) || 0;
                    new_row = $(product_modifier_template({
                        mod_key: mod_key
                ...
                reloadStockTable = function(existing_stock) {					
					...
					product = $('.settings').serializeObject().store_product_field;
					//settings ajax-validate = form name
				...
				$("#store_product_stock .store_track_stock input:checkbox").change(function() {
				...
				$("#store_product_stock .checkall_stock_publish").change(function() {
