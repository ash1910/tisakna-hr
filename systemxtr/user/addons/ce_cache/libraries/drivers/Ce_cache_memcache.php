...
class Ce_cache_memcache extends Ce_cache_driver
{
	...
	private function setup()
	{
		if ( ! $this->is_setup ) //if not already setup
		{
			...
			if ( ! empty( ee()->config->_global_vars[ 'ce_cache_memcache_servers' ] ) ) //first check the global array
			{
				$config = ee()->config->_global_vars[ 'ce_cache_memcache_servers' ];
			}
			else if ( ee()->config->item( 'ce_cache_memcache_servers' ) != false ) //then check the config
			{
				$config = ee()->config->item( 'ce_cache_memcache_servers' );
			}
