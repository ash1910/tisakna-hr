...
// Stop EE3's click event on all table rows from messing with Matrix's settings checkboxes.
...
MatrixConf = function(namespace, celltypes, colInfo, colSettings){
	...
	var setContainerWidth = function(){
		...
		var containerWidth = MatrixConf.EE1 ? $window.width() - 48 : obj.dom.$settingsContainer.width() - 22;
	...
	obj.addCol = function(index){
		...
		// remove btn
		// <a class="matrix-btn matrix-add" title="'+MatrixConf.lang.add_col+'"></a>
		$('<td class="matrix-breakdown"><a class="matrix-btn matrix-remove" title="'+MatrixConf.lang.delete_col+'" /></td>').appendTo(obj.dom.$trs[obj.colSettings.length+2]);
...
MatrixConf.Col = function(field, index, id, type){
	...
	obj.dom.$removeBtn = $('> a.matrix-remove', obj.dom.$tds[obj.dom.$tds.length-1]);
