$(document).ready(function (){
    ...
    /**
     * Build custom Zenbu URLs
     * by providing only the variable part
     */
    function zenbuUrl(method)
    {
        var zenbuUrl    = EE.BASE.replace(/\?(.*)/g, '?/cp/addons/settings/zenbu/');
        var baseUriVars = '&' + EE.BASE.substr(EE.BASE.indexOf('?') + 1);

        return zenbuUrl + method + baseUriVars;
    }
    ...
    parseSearchFilters(zenbuUrl('fetch_cached_temp_filters')); // Does updateSelect2() as well
    ...
    /**
     * Show the sidebar on page load if
     * cache is storing sidebar_hidden as FALSE
     */
    if(sidebar.hasClass('show-sidebar-on-load'))
    {
        $("div.mainZenbuArea").closest(".w-16").removeClass("w-16").addClass(function(){
            // Show the sidebar before applying the w-12 class
            $(this).next('.col.w-4').show();
            return $(this).attr('class') + ' w-12';
        });
    }
    ...
    $("body").delegate("a.show-sidebar", "click", function(e){
        ...
        if($(this).hasClass("showing"))
        {
            ...
            $(this).closest('li').find('i.fa').removeClass("fa-arrow-left").addClass("fa-bars");
        ...
        else
        {
            ...
            $(this).closest('li').find('i.fa').removeClass("fa-bars").addClass("fa-arrow-left");
    ...
    */
    ...
    $("body").delegate(".addFilterRule", "click", function (e) {
        ...
        // Removing Select2 elements from cloned row
        filterRow.find("select").each(function() {
            $(this).next('.select2-container').remove();
            if ($(this).data('select2')) {
                $(this).select2('destroy').show();
            }
        });
     ...
     * iframe Modal window
     */
    $("body").delegate(".iframe-modal", "click", function(e){
        e.preventDefault();
        var theUrl = $(this).attr('data-url');
        var theRel = $(this).attr('rel');
        var windowHeight = $(window).height();
        $('div.' + theRel).css({ top: '10px' });
        $('div.' + theRel).find('div.box').css({ height: windowHeight - 75 }); // 75px removed to count in that huge modal title bar at top 10px spacer
        $('div.' + theRel).find('div.box').html('<iframe src="' + theUrl + '" style="width: 100%; height: 100%; border: none"></iframe>');
    });
    ...
    function processBasedOnSecondFilter()
    {
        $("tr.filter-params").each(function(){
            ...
            // Likely the 3rd field is already showing
            // but coming from isempty/isnotempty it would
            // be hidden, so this re-displays the field.
    ...
    {
        originalOrderbyFilterVal = $("tr.limit-and-orderby").find("td.orderbyFilter select").val();
        ...
        $("tr.limit-and-orderby").find("td.orderbyFilter").replaceWith(orderbyFilter);

        // The orderby dropdown has changed.
        // If the original value is still present
        // in this new dropdown, select it. If not
        // natually let the dropdown select its first option
        orderbyFilter.find('option').each(function() {
           if(this.value == originalOrderbyFilterVal)
           {
                orderbyFilter.find("select, input").val(originalOrderbyFilterVal);
           }
        });
    }
    ...
    $("body").delegate("form#saveSearch", "submit", function(e){
        ...
        {
            $("button.submit").removeClass('work').find("span").hide().eq(0).show();
            return;
        }
        ...
        $.ajax({
                    ...
                    success: function(results){
                            ...
                            $("form#saveSearch").find("button.submit").removeClass('work');
    ...
    {
        $("h2#savedSearchesListTitle").show();
        $("ul#savedSearchesList").show().empty();
        ...
        $(data.items).each(function(index, item) {
            $("ul#savedSearchesList").append(
                $('<li></li>').append(
                    $('<a></a>').text(item.label).attr('href', baseUrl + 'fetch_search_filters&searchId=' +item.id + '&S=' + data.base_url.session_id)
                )
            );
        });
    }
     ...
     */
    $("body").delegate("a[href*='zenbu/fetch_search_filters&searchId=']", "click", function(e){
        e.preventDefault();
    ...
    function parseSearchFilters(theUrl)
    {
        ...
        $.ajax({
                ...
                success: function(results){
                    ...
                    if(results.length > 0)
                    {
                        ...
                        $.each(results, function(item, filter){
                            ...
                            if(filter.filterAttribute1 == sectionName)
                            {
                                $("div.zenbu-filters select#section_select").val(sectionId);
                                $(".entryType_select").hide().find("select").attr('disabled', 'disabled');
                ...
                error: function(results){
                        console.log('Error in parseSearchFilters');
                        console.log(results.statusText);
                        console.log(results.responseText);
                        $(".loading").hide();
                }
