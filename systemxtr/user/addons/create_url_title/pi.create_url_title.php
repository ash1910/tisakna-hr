...
class Create_url_title
{
	...
	public function Create_url_title($tagdata = '')
	{
		...
		if (in_array(ee()->TMPL->fetch_param('separator'), array('dash', '-')))
		{
			$separator = 'dash';
		}
		elseif (ee()->TMPL->fetch_param('separator') == '_')
		{
			$separator = '_';
		}
		else
		...
		$this->return_data = url_title(
			trim($tagdata),
			$separator,
			(preg_match('/0|no|off|n/i', ee()->TMPL->fetch_param('lowercase'))) ? FALSE : TRUE
		);
	}
