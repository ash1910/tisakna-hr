...
class Ce_cache_db extends Ce_cache_driver
{
	...
	public function is_supported()
	{
		...
		return ee()->db->table_exists( 'ce_cache_db_driver' );
	}
	...
	public function set( $id, $content = '', $seconds = 360 )
	{
		...
		$sql = ee()->db->insert_string( 'exp_ce_cache_db_driver', $data );
	 ...
	 *
	 * @param int $id The cache item's id.
	 * @return bool
	 */
	public function delete( $id )
	{
		ee()->db->query( 'DELETE FROM exp_ce_cache_db_driver WHERE id = ?', array( $id ) );
		return true;
	}
	...
	{
		return ee()->db->empty_table( 'exp_ce_cache_db_driver' );
	}

	/**
	 * Retrieves all of the cached items at the specified relative path.
	 *
	 * This method differs from the other drivers, as it also returns the metadata for the objects, without the 'content' item.
	 * Most of the other driver implementations only return the item id.
	 *
	...
	public function get_all( $relative_path )
	{
		...
		made
		FROM exp_ce_cache_db_driver
		WHERE SUBSTRING( id, 1, " . strlen( $relative_path )  . ") = '" . ee()->db->escape_str( $relative_path ) . "'
		ORDER BY id ASC" );
